...
mod test {
    ...
    fn verify_keys_test(keys: Vec<Vec<u8>>, expected_result: Vec<Option<Vec<u8>>>) {
        ...
        let expected_hash = [
            193, 133, 129, 102, 77, 34, 77, 66, 159, 205, 217, 149, 22, 21, 72, 60, 13, 214, 130,
            115, 40, 237, 35, 101, 221, 203, 55, 0, 155, 110, 73, 193,
        ];
    ...
    fn empty_proof() {
        ...
        assert_eq!(
            ...
            Some(&Op::Push(Node::Hash([
                63, 80, 78, 216, 65, 141, 28, 234, 117, 252, 18, 4, 73, 82, 16, 98, 78, 240, 94,
                51, 96, 159, 182, 38, 57, 205, 0, 194, 130, 48, 70, 234
            ])))
        );
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::KVHash([
                233, 85, 196, 12, 165, 104, 114, 181, 183, 33, 212, 210, 120, 162, 148, 87, 124,
                119, 9, 30, 157, 16, 25, 146, 128, 176, 218, 99, 99, 10, 242, 123
            ])))
        );
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                142, 144, 55, 22, 52, 234, 222, 80, 93, 55, 254, 242, 5, 216, 241, 234, 46, 43,
                191, 5, 137, 229, 55, 79, 164, 246, 141, 72, 128, 1, 12, 123
            ])))
        );
    ...
    fn doc_proof() {
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                233, 121, 154, 230, 92, 24, 199, 14, 247, 118, 38, 71, 10, 248, 63, 142, 7, 92,
                158, 38, 249, 42, 200, 190, 102, 1, 80, 191, 224, 159, 243, 107
            ])))
        );
        assert_eq!(iter.next(), Some(&Op::Child));
        assert!(iter.next().is_none());
        ...
        assert_eq!(
            bytes,
            vec![
                3, 0, 1, 1, 0, 0, 0, 1, 1, 3, 0, 1, 2, 0, 0, 0, 1, 2, 16, 3, 0, 1, 3, 0, 0, 0, 1,
                3, 3, 0, 1, 4, 0, 0, 0, 1, 4, 16, 17, 2, 233, 85, 196, 12, 165, 104, 114, 181, 183,
                33, 212, 210, 120, 162, 148, 87, 124, 119, 9, 30, 157, 16, 25, 146, 128, 176, 218,
                99, 99, 10, 242, 123, 16, 1, 233, 121, 154, 230, 92, 24, 199, 14, 247, 118, 38, 71,
                10, 248, 63, 142, 7, 92, 158, 38, 249, 42, 200, 190, 102, 1, 80, 191, 224, 159,
                243, 107, 17
            ]
        );
    ...
    fn range_proof() {
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                47, 43, 94, 88, 30, 240, 150, 161, 122, 21, 215, 169, 57, 123, 60, 17, 0, 243, 63,
                21, 63, 189, 151, 245, 230, 61, 123, 162, 46, 246, 170, 50
            ])))
        );
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::KVHash([
                170, 95, 49, 56, 206, 134, 159, 192, 158, 195, 42, 174, 29, 171, 65, 180, 160, 88,
                216, 128, 105, 108, 71, 155, 236, 160, 154, 109, 61, 54, 122, 59
            ])))
        );
        assert_eq!(iter.next(), Some(&Op::Parent));
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                149, 159, 135, 5, 163, 151, 151, 136, 25, 156, 67, 124, 203, 92, 145, 32, 243, 189,
                32, 63, 59, 40, 50, 71, 7, 196, 181, 25, 217, 208, 201, 185
            ])))
        );
        assert_eq!(
            iter.next(),
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                183, 71, 90, 162, 111, 105, 121, 247, 174, 235, 22, 217, 40, 174, 78, 196, 215, 19,
                191, 138, 105, 227, 5, 93, 61, 59, 163, 214, 69, 80, 217, 130
            ])))
        );
        assert_eq!(iter.next(), Some(&Op::Child));
        assert_eq!(iter.next(), Some(&Op::Child));
