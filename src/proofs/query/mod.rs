...
mod test {
    ...
    fn verify_keys_test(keys: Vec<Vec<u8>>, expected_result: Vec<Option<Vec<u8>>>) -> Result<()> {
        ...
        let expected_hash = [
            203, 93, 80, 123, 188, 135, 33, 151, 94, 129, 223, 99, 201, 49, 250, 219, 243, 186, 91, 90, 234, 142, 8, 166, 169, 45, 117, 134, 198, 211, 44, 91
        ];
    ...
    fn empty_proof() -> Result<()> {
        ...
        assert_eq!(
            ...
            Some(&Op::Push(Node::Hash([
                33, 81, 116, 89, 129, 22, 194, 110, 199, 17, 207, 157, 223, 202, 112, 17, 222, 63, 47, 208, 175, 45, 231, 59, 52, 165, 135, 162, 44, 248, 38, 60
            ])))
        );
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::KVHash([
                134, 186, 111, 18, 167, 112, 88, 119, 247, 214, 178, 67, 227, 248, 182, 140, 75, 232, 9, 76, 125, 133, 253, 172, 117, 123, 126, 68, 239, 130, 117, 26
            ])))
        );
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                181, 109, 61, 183, 207, 2, 197, 66, 142, 22, 252, 125, 105, 67, 47, 138, 81, 26, 53, 66, 118, 239, 139, 163, 167, 135, 157, 55, 240, 253, 87, 1
            ])))
        );
    ...
    fn doc_proof() -> Result<()> {
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                238, 164, 159, 94, 201, 115, 228, 153, 230, 187, 22, 34, 76, 55, 52, 60, 141, 91, 85, 245, 172, 155, 139, 242, 0, 133, 186, 131, 156, 76, 195, 169
            ])))
        );
        assert_eq!(iter.next(), Some(&Op::Child));
        assert!(iter.next().is_none());
        ...
        assert_eq!(
            bytes,
            vec![
                3, 1, 1, 0, 1, 1, 3, 1, 2, 0, 1, 2, 16, 3, 1, 3, 0, 1, 3, 3, 1, 4, 0, 1, 4, 16, 17, 2, 134, 186, 111, 18, 167, 112, 88, 119, 247, 214, 178, 67, 227, 248, 182, 140, 75, 232, 9, 76, 125, 133, 253, 172, 117, 123, 126, 68, 239, 130, 117, 26, 16, 1, 238, 164, 159, 94, 201, 115, 228, 153, 230, 187, 22, 34, 76, 55, 52, 60, 141, 91, 85, 245, 172, 155, 139, 242, 0, 133, 186, 131, 156, 76, 195, 169, 17
            ]
        );
    ...
    fn range_proof() -> Result<()> {
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                94, 198, 237, 10, 79, 103, 114, 39, 160, 19, 181, 109, 246, 251, 179, 198, 33, 248, 231, 197, 252, 144, 180, 229, 109, 103, 13, 164, 141, 68, 68, 118
            ])))
        );
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::KVHash([
                128, 13, 134, 78, 50, 129, 24, 24, 86, 74, 190, 178, 206, 243, 137, 65, 188, 69, 232, 49, 209, 160, 228, 249, 229, 30, 120, 84, 164, 38, 114, 79
            ])))
        );
        assert_eq!(iter.next(), Some(&Op::Parent));
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                245, 77, 101, 22, 93, 9, 164, 109, 90, 74, 213, 158, 152, 144, 101, 210, 187, 110, 73, 225, 244, 121, 205, 179, 59, 101, 106, 209, 97, 75, 48, 163
            ])))
        );
        assert_eq!(
            iter.next(),
        ...
        assert_eq!(
            iter.next(),
            Some(&Op::Push(Node::Hash([
                117, 139, 80, 138, 2, 145, 233, 59, 103, 243, 112, 241, 204, 99, 118, 250, 141, 170, 156, 129, 214, 127, 166, 20, 163, 229, 141, 252, 155, 191, 122, 119
            ])))
        );
        assert_eq!(iter.next(), Some(&Op::Child));
        assert_eq!(iter.next(), Some(&Op::Child));
